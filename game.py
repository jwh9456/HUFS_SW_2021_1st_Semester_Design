...
def play_round(hands, players, discard=None, first_player=0, invalid_action='pass'):
    ...
    while True:
        ...
        # args =[[이전에 낸 카드],[손에 들고 있는 카드],[버린 카드],(각자 가지고 있는 카드의 개수)]
        ...
        try:
            ...
            if invalid_action == 'pass':
                ...
                raise InvalidAction("player {0} raised {1}: '{2}'".format(index, html.escape(e.__class__.__name__), e),
                                    call_str)
        ...
        if prev is None:  # 전에 낸 카드가 없다면
            ...
            if not common.is_valid_play(prev, play):
                ...
                if invalid_action == 'pass':
                    ...
                    raise InvalidAction("player {0} attempted invalid play {1} -> {2}".format(index, prev, play),
                                        call_str)
...
def play_game(invalid_action='raise', initial_deal=None):  # invalid_action = 'raise'
    ...
    player1 = participation('player1')
    player2 = participation('player2')
    player3 = participation('player3')
    player4 = participation('player4')
    ...
    lp_hist.append(lp)
    players[lp].score += 10
    print("{0}의 점수는 {1} 입니다.\n".format(players[lp].name, players[lp].score))
    while not game_over:
        print("라운드를 다시 시작합니다.")
...
for i in range(4):
    print(players[i])
    if result[i].score > result[maxindex].score:
        maxindex = i

result[i].rank = "대부호"

for i in range(4):
    print("{0}의 계급: ".format(i, result[i].rank))
