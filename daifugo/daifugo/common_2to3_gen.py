...
def cards_by_index(cardset, index):   #카드를 rank 또는 suit에 따라 dic를 만들어 준다
    """
    Organize cards by rank (index 0) or suit (index 1)
    """
    retval = defaultdict(list)       #디폴트 값이 list인 딕셔너리 -> 리스트 함수 사용 가능
    for card in cardset:
        rank = card[index]
        retval[rank].append(card)    # 딕셔너리이름[key].append(value)
    return retval

def card_value(card):      #카드를 매개변수로 받으면 그 카드의 rank를 알려준다.
    """
...
def straights(cards):      #straights 판단해주는 함수
    ...
    num_cards = len(cards)
    retval = []
    retval_j = []                            #조커가 있는 경우 straights의 경우의 수를 넣을 lsit
    ...
    if 'BB' in cards:                             #조커가 있는 경우 straights판단
        for i in range(3, num_cards+1, 1):        #연속된 세 자리 straights부터해서 카드의 갯수 만큼의 straights판단
...
def is_valid_play(prev, play, debug=False):  #이전 패와 비교해서 지금 내려는 카드가 타당한지 판단
    """
